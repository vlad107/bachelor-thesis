...
В общем случае возниают следующие проблемы:
\begin{itemize}
	\item Во-первых для каждого пути в графе доказательства композиция унификаций может давать уникальный литерал.
    \item Во-вторых может расти глубина литерала.
\end{itemize}

Для того, чтобы корректно обходить обе проблемы, заметим, что мы доказываем \emph{полноту по опровержению}. Иначе говоря, нам важно лишь доказать, что если $\bot$ выводится в исчислении \emph{Conflict Resolution}, то существует доказательство $\bot$ и в исчислении \emph{Expertised Conflict Resolution}. Очевидным образом по индукции доказывается, что если множество клозов противоречиво, то из любого клоза мы можем выкинуть произвольное подмножество литералов, и множество останется противоречивым. 
